...
停止の設計（ユーザー指定）: アラームを止める手段はアプリ内の「スライドして止める」だけにする。システムの停止ボタンは現在の 1 件しか止められず、1 分後に次の連鎖アラーム（既定 10 件、設定で 5 / 10 / 20 分）が鳴る。Open ボタンまたはアイコンからアプリを開くと `AlarmStopView` が全画面で開き、本人の宣言音声をフォアグラウンドでループ再生する。横スワイプのスライダーを最後まで動かした時だけ残りのアラームを全て取り消し、再生を止めて NoonFlow N0 → N1 に進む。スライドせずにアプリを閉じた場合は連鎖が続く。連鎖が尽きた後は通常の行動時刻通知に戻る。
