...
2. **ブランチ**: `task/<番号>-<短い名前>` を main から切る。並列作業は git worktree（`EnterWorktree`）で分離する。
3. **実装**: `scope` の範囲だけを実装する。`non_scope` に触れない。文言はすべて `DialogueCopy` / `NotificationCopy` に置き、画面や ViewModel に直書きしない。
4. **検証**: `verify_commands` を全て実行し、出力を要約せずに `docs/PROGRESS.md` に貼る。失敗はそのまま記録して Fixer に回すか、自分で直す。
...
7. **PR**: main への PR を作り、Reviewer セッションが `/code-review high` と task-review ワークフローを実行する。指摘の修正は同じブランチで行い、人間がマージする。
...
|---|---|---|
| P0 | task_003 ∥ task_004 | task_002 完了後。fm-probe は Mac、SpeechSpike は実機 |
| P1 | task_005 ∥ task_006 | task_002 完了後。パッケージと App 側で衝突しない |
| P2 | task_007（task_004 完了後）∥ task_005 の残り | 音声スタックはフローと独立 |
| P3 | task_014 ∥ task_019 ∥ task_021（任意） | task_013 完了後。Phase 1.5 のドッグフーディング期間に走らせる |
| P4 | task_017 ∥ task_018 | task_016 完了後 |

直列必須: task_008 → 009 → 010 → 011 → 012 → 013（SessionViewModel を共有するため）。
...
| G1 コアループ | task_013 完了時 | `.claude/workflows/phase-gate.js`（各 acceptance check を 1 エージェントが pass / fail / needs-human 判定）+ 人間の実機チェック | check_001〜013・015・024 が pass または人間確認済み。TestFlight 内部配布済み |
| G2 AI 層 | task_017 完了時 | 同上 + fm-probe の通過率 | check_005・006・007・014 が pass。自動判定の通過率 90% 以上、置換率が記録されている |
| G3 リリース | task_020 前 | 人間 + `/security-review` | check_016〜025 が pass。外部テスト 1 週間の記録あり |
...
| 部品 | 作成タスク | 内容 |
|---|---|---|
| `CLAUDE.md`（SAYDO 直下） | task_001 | モジュール構成、3 つの検証コマンド、Executor 固定指示、企画原則 §22 の要約 |
| `scripts/test-core.sh` / `build-ios.sh` / `test-ios.sh` | task_001 | 唯一の検証コマンド。エージェントは xcodebuild を直接叩かない |
| `docs/PROGRESS.md` | task_001 | セッション間の状態引き継ぎ。1 タスク 1 エントリ |
| `.claude/workflows/plan-review.js` | 作成済み | 計画書の敵対的レビュー（5 レンズ → 統合 → 反証検証）。`Workflow({name: "plan-review"})` で再実行 |
| `.claude/workflows/task-review.js` | task_001 | PR 差分を 4 観点（正しさ / 企画原則 / Swift 6 並行性 / テスト充足）で並列レビューし、各指摘を反証検証 |
| `.claude/workflows/copy-audit.js` | task_005 | `DialogueCopy` / `NotificationCopy` / 画面の全文字列を抽出し、Guardrails とトーンを監査 |
| `.claude/workflows/phase-gate.js` | task_013 | `docs/acceptance-checks.json` の各 check を 1 エージェントが判定し、needs-human を人間向けチェックリストとして出力 |
